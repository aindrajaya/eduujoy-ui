...
/**
 * Generate cache key for transcript or search caching
 * @param {string} type - Cache type (e.g., 'transcript', 'youtube-search', 'metadata')
 * @param {string} identifier - Video ID or search query
 * @returns {string} Cache key
 */
export function generateCacheKey(type, identifier) {
  // Handle legacy single-parameter calls (backwards compatibility)
  if (!identifier) {
    return `transcript_${type}`;
  }
  // New format with type and identifier
